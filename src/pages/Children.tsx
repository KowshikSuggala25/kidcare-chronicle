...
const Children = () => {
    ...
    const fetchChildren = async () => {
      ...
      try {
        ...
        if (userProfile.role === 'parent') {
          q = query(collection(db, 'children'), where('parentId', '==', userProfile.id));
        } else {
  ...
  const handleChildAdded = async (childData: Omit<Child, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground">
              {userProfile?.role === 'parent' ? 'Your Children' : 'All Patients'}
            </h1>
        ...
        {children.length === 0 ? (
          <div className="text-center py-12 bg-gradient-card rounded-lg border">
            ...
            <h3 className="mt-4 text-lg font-medium text-foreground">
              {userProfile?.role === 'parent' ? 'No children added yet' : 'No patients found'}
            </h3>
